/*:
@target MV MZ
@plugindesc スキルツリー アイコン拡張 v1.0.0
@author うなぎおおとろ
...
@param OpenedImageFileName
@type file
@dir img
@desc
習得済みスキルに追加する画像のファイル名を指定します。

@param OpenedImageXOfs
@type number
@default 0
@desc
習得済みスキルに追加する画像のX座標オフセットを指定します。

@param OpenedImageYOfs
@type number
@default 0
@desc
...
(() => {
    ...
    const OpenedImageXOfs = parseInt(params["OpenedImageXOfs"]);
    const OpenedImageYOfs = parseInt(params["OpenedImageYOfs"]);
    const IconFontSize = parseInt(params["IconFontSize"]);

    const skillTreeParams = PluginManager.parameters(SkillTreePluginName);
    const IconWidth = parseInt(skillTreeParams["IconWidth"]);
    const IconHeight = parseInt(skillTreeParams["IconHeight"]);
    const RectImageFileName = skillTreeParams["RectImageFileName"];
    const ViewRectColor = skillTreeParams["ViewRectColor"];
    const ViewRectOfs = parseInt(skillTreeParams["ViewRectOfs"]);
    ...
    Scene_SkillTree.prototype.isReady = function() {
        ...
        if (OpenedImageFileName) {
            const openedImage = ImageManager.loadBitmap("img/", OpenedImageFileName);
    ...
    SkillTreeNodeInfo.prototype.iconExBitmap = function(opened) {
        ...
        const textWidth = IconWidth - 32 - dx - 5;
        dstBitmap.fontSize = IconFontSize;
        if (ChangeOpenedTextColor && opened) {
            const tmpTextColor = dstBitmap.textColor;
            if (Utils.RPGMAKER_NAME === "MZ") {
                dstBitmap.textColor = ColorManager.crisisColor();
            } else {
                const dummyWindow = new Window_Base(0, 0, 0, 0);
                dstBitmap.textColor = dummyWindow.crisisColor();
            }
            dstBitmap.drawText(this.skill().name, 32 + dx + 5, dy, textWidth, 32, "left");
    ...
    SkillTreeView.prototype.viewDrawNode = function(bitmap) {
        for (const node of Object.values(this._skillTreeManager.getAllNodes())) {
            ...
            if (node.isOpened()) {
                if (EnableOpenedImage) {
                    const x = px + OpenedImageXOfs;
                    const y = py + OpenedImageYOfs;
                    const openedImage = ImageManager.loadBitmap("img/", OpenedImageFileName);
