...
const SelfVariablePluginName = document.currentScript ? decodeURIComponent((document.currentScript as HTMLScriptElement).src.match(/^.*\/(.+)\.js$/)![1]) : "SelfVariable";

var globalActiveInterpreter: Game_Interpreter | undefined;
var globalActiveEvent: Game_Event | undefined;

namespace SelfVariable {
    export type VariableType = "SelfVariable" | "FloatVariable" | "CommonVariable";
    ...
    if (typeof PluginManager.registerCommand !== "undefined") {
        function getMapId(interpreter: Game_Interpreter, args: any): number {
            ...
            if (mapId === 0) return interpreter.mapId();
            return mapId;
        }

        function getEventId(interpreter: Game_Interpreter, args: any): number {
        ...
        PluginManager.registerCommand(SelfVariablePluginName, "GetSelfVariableValue", function(this: Game_Interpreter, args: any) {
            ...
            const value = $gameVariables.selfVariableValue(key as any);
            $gameVariables.setValue(destVariableId, value);
        });

        PluginManager.registerCommand(SelfVariablePluginName, "SetSelfVariableValue", function(this: Game_Interpreter, args: any) {
            const mapId = getMapId(this, args);
            const eventId = getEventId(this, args);
            const selfVariableId = parseInt(args.SelfVariableId);
            ...
            $gameVariables.setSelfVariableValue(key as any, value);
        });

        PluginManager.registerCommand(SelfVariablePluginName, "SetSelfVariableValueByEventTags", function(this: Game_Interpreter, args: any) {
        ...
        PluginManager.registerCommand(SelfVariablePluginName, "ClearSelfVariables", function(this: Game_Interpreter, args: any) {
            const mapId = getMapId(this, args);
            let eventId;
            if (args.EventSpecification !== "false") {
                eventId = getEventId(this, args);
            }
            let selfVariableId: number | undefined;
            if (args.SelfVariableId) {
                selfVariableId = parseInt(args.SelfVariableId);
                if (selfVariableId === 0) selfVariableId = undefined;
            }
            $gameVariables.clearSelfVariables(mapId, eventId, selfVariableId);
        });

        PluginManager.registerCommand(SelfVariablePluginName, "GetExSelfSwitchValue", function(this: Game_Interpreter, args: any) {
            const mapId = getMapId(this, args);
            const eventId = getEventId(this, args);
            ...
            const value = $gameSwitches.exSelfSwitchValue(key as any);
            $gameSwitches.setValue(destSwitchId, value);
        });

        // NOTE: SetExSelfVariableValueは誤記だが旧版との互換性を考慮してこのままとする。
        PluginManager.registerCommand(SelfVariablePluginName, "SetExSelfVariableValue", function(this: Game_Interpreter, args: any) {
            const mapId = getMapId(this, args);
            const eventId = getEventId(this, args);
            const exSelfSwitchId = parseInt(args.ExSelfSwitchId);
            ...
            $gameSwitches.setExSelfSwitchValue(key as any, value);
        });

        PluginManager.registerCommand(SelfVariablePluginName, "SetExSelfSwitchValueByEventTags", function(this: Game_Interpreter, args: any) {
            const eventTags = JSON.parse(args.EventTags);
            const exSelfSwitchId = parseInt(args.ExSelfSwitchId);
            let value = args.Value === "true";
            const srcSwitchId = parseInt(args.SrcSwitchId);
        ...
        PluginManager.registerCommand(SelfVariablePluginName, "ClearExSelfSwitches", function(this: Game_Interpreter, args: any) {
            const mapId = getMapId(this, args);
            let eventId;
            if (args.EventSpecification !== "false") {
                eventId = getEventId(this, args);
            }
            let exSelfSwitchId: number | undefined;
            if (args.ExSelfSwitchId) {
                exSelfSwitchId = parseInt(args.ExSelfSwitchId);
                if (exSelfSwitchId === 0) exSelfSwitchId = undefined;
            }
    ...
    class ErrorManager {
        ...
        static invalidSelfVariableAccess(variableId: number) {
            let errorMessage;
            if (ErrorLanguage === "ja") {
        ...
        static invalidCommonVariableAccess(variableId: number) {
            let errorMessage;
            if (ErrorLanguage === "ja") {
    ...
    export class SelfVariableOrExSwitchUtils {
        static isDebugScene() {
            if ((SceneManager as any)._scene instanceof Scene_Debug) return true;
            return false;
        }

        static currentExSelfSwitchKey(id: number) {
        ...
        static checkPrefix(name: string, index: number, expectedPrefix: string) {
    ...
    Game_Variables.prototype.setValue = function(this: Game_Variables, variableId, value) {
        if (this.isSelfVariable(variableId)) {
    ...
    Game_Variables.prototype.selfVariableValue = function(key) {
        return this._selfVariablesData[key as any] || 0;
    };
    ...
    Game_Variables.prototype.setSelfVariableValue = function(key, value) {
        ...
        if (variableId > 0 && variableId < $dataSystem.variables.length) {
            ...
            this._selfVariablesData[key as any] = value;
            this.onChange();
        }
    };
    ...
    Game_Switches.prototype.value = function(switchId) {
        if (this.isExSelfSwitch(switchId)) {
            if (SelfVariableOrExSwitchUtils.isDebugScene()) return false;
    ...
    Game_Interpreter.prototype.mapId = function(this: Game_Interpreter) {
        return this._mapId;
    };

    Game_Interpreter.prototype.eventId = function(this: Game_Interpreter) {
        return this._eventId;
    };

    Game_Interpreter.prototype.selfVariableOrExSwitchKey = function(this: Game_Interpreter, id) {
    ...
    Game_Event.prototype.initialize = function(...args: any[]): void {
    ...
    Game_Event.prototype.addEventTag = function(eventTag: string): void {
