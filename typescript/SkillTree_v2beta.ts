...
declare function loadSkillTreeConfig(): any;

declare interface Scene_Boot {
    initSkillTreeConfig(): void;
    loadSkillTreeMap(): void;
}

declare interface Game_Party {
    makeMenuActorNextOrPreviousWhenSkillTree(isNext: boolean): void;
    gainSp(sp: number): void;
}

declare interface Window_MenuCommand {
    isEnabledSkillTree(): boolean;
}

declare interface Game_Actor {
    gainSp(sp: number): void;
    getLevelUpSp(level: number): void;
}
...
declare interface Game_Temp {
    _enableGetSpWhenLevelUp: boolean;
    _prevLevel: number | null;

    enableGetSpWhenLevelUp(): boolean;
    setEnableGetSpWhenLevelUp(enableGetSpWhenLevelUp: boolean): void;
    prevLevel(): number | null;
    setPrevLevel(prevLevel: number | null): void;
}

declare interface Game_Party {
    itemCount(itemId: number): number;
    weaponCount(weaponId: number): number;
    armorCount(armorId: number): number;
}


const SkillTreePluginName = document.currentScript ? decodeURIComponent((document.currentScript as HTMLScriptElement).src.match(/^.*\/(.+)\.js$/)![1]) : "SkillTree";

let $skillTreeData!: SkillTree.SkillTreeData;
let $skillTreeConfigLoader!: SkillTree.SkillTreeConfigLoader;
const $skillTreeMapLoaders: Map<string, SkillTree.SkillTreeMapLoader> = new Map();

function skt_open(actorId: number) {
    $gameParty.setMenuActor($gameActors.actor(actorId)!);
    SceneManager.push(SkillTreeClassAlias.Scene_SkillTree);
}

function skt_gainSp(actorId: number, value: number) {
    const actor = $gameParty.members().find((actor: Game_Actor) => actor.actorId() === actorId);
    if (actor) actor.gainSp(value);
}

function skt_skillReset(actorId: number) {
...
function skt_totalSp(actorId: number, variableId: number) {
    const totalSp = $skillTreeData.totalSpAllTypes(actorId);
    $gameVariables.setValue(variableId, totalSp)
}

function skt_learn(actorId: number, typeName: string, skillId: number, force = false) {
    const types = $skillTreeData.types(actorId);
    const findedType = types.find((type: SkillTree.SkillDataType) => type.skillTreeName() === typeName);
...
function skt_enableType(actorId: number, typeName: string) {
    const types = $skillTreeData.types(actorId);
    let targetType = null;
...
function skt_disableType(actorId: number, typeName: string) {
    const types = $skillTreeData.types(actorId);
    let targetType = null;
    for (const type of types) {
        if (type.skillTreeName() === typeName) {
...
function skt_migrationType(actorId: number, fromTypeName: string, toTypeName: string, reset: boolean) {
    let dstType = null;
    let srcType = null;
    const types = $skillTreeData.types(actorId);
    for (const type of types) {
...
namespace SkillTree {
    ...
    if (Utils.RPGMAKER_NAME === "MZ") {
        PluginManager.registerCommand(SkillTreePluginName, "StartSkillTreeScene", (args: any) => {
        ...
        PluginManager.registerCommand(SkillTreePluginName, "SkillLearn", (args: any) => {
        ...
        PluginManager.registerCommand(SkillTreePluginName, "GainSp", (args: any) => {
        ...
        PluginManager.registerCommand(SkillTreePluginName, "GetTotalSp", (args: any) => {
        ...
        PluginManager.registerCommand(SkillTreePluginName, "SkillReset", (args: any) => {
            const params = PluginParamsParser.parse(args, { ActorId: "number" });
            skt_skillReset(params.ActorId);
        });

        PluginManager.registerCommand(SkillTreePluginName, "SetTypeEnableOrDisable", (args: any) => {
        ...
        PluginManager.registerCommand(SkillTreePluginName, "MigrationType", (args: any) => {
    ...
    export class SkillTreeNodeInfo {
        protected _actorId: number;
        protected _skillId: number;
        protected _needCost: any;
        protected _iconData: any;
        protected _helpMessage: string;
        protected _learnCommonEventId: number;
        protected _forgetCommonEventId: number;

        constructor(actorId: number, skillId: number, needCost: any, iconData: any, helpMessage: string) {
        ...
        learnSkill() {
            if (this.learnCommonEventId() > 0) return;
            this.actor().learnSkill(this._skillId);
        }

        forgetSkill() {
            if (this.forgetCommonEventId() > 0) return;
        ...
        crisisColor() {
            ...
            const dummyWindow: any = new Window_Base(0, 0, 0, 0);
            return dummyWindow.crisisColor();
        }

        costTypes(): string[] {
            if (!this._needCost) return [];
            return Object.keys(this._needCost);
        }

        needSp() {
            if (!this._needCost) return null;
    ...
    export class SkillTreeNode {
        ...
        protected _parents: SkillTreeNode[];
        protected _childs: SkillTreeNode[];
        protected _info: SkillTreeNodeInfo | null;
        protected _opened: boolean;
        protected _point: Point | null;
        protected _reservedPoint: Point | null;
        protected _needStartCommonEventId: number;
        ...
        setReservedPoint(point: Point) {
            this._reservedPoint = point;
        }

        getAllChilds(): SkillTreeNode[] {
            let allChilds: SkillTreeNode[] = [this];
        ...
        addChild(child: SkillTreeNode) {
            if (!child) throw new Error("child is none.");
            child.parents.push(this);
            this._childs.push(child);
        }

        setup(info: SkillTreeNodeInfo | null) {
            this._info = info;
        }
        ...
        setOpenedStatus(openStatus: boolean) {
            ...
            for (const costType of this.costTypes()) {
                if (costType === "sp") {
                    ...
                    for (const needItem of this.needItems()) {
                        ...
                        if (needCount > itemInfo.partyItemCount()) return false;
                    }
                }
            }
            return true;
        ...
        makePointNode(x: number, y: number, mode: string) {
        ...
        costTypes(): string[] {
            if (!this._info) throw new Error("_info is null.");
    ...
    export class SkillTreeTopNode extends SkillTreeNode {
        constructor() {
            super(null);
            this._opened = true;
        ...
        skillReset() {
            for (const child of this._childs) {
                if (!(child instanceof SkillTreeTopNode)) continue;
                if (child.isOpened()) child.skillReset();
            }
        }
    }

    export class SkillDataType {
        protected _skillTreeName: string;
        protected _actorId: number;
        ...
        constructor(skillTreeName: string, actorId: number, message: string, helpMessage: string, enabled: boolean) {
    ...
    export class SkillTreeMapLoader {
    ...
    export class SkillTreeConfigLoader {
        ...
        loadTypes(actorId: number): SkillDataType[] {
        ...
        loadTypesWhenSkillTreeManagementByActor(actorId: number): SkillDataType[] {
            let cfgTypes = null;
            let typesArray = [];
        ...
        loadTypesWhenSkillTreeManagementByClasses(actorId: number): SkillDataType[] {
            let typesArray = [];
            for (const cfg of this._configData.skillTreeTypes) {
        ...
        loadSkillTreeNodes(type: SkillDataType) {
            ...
            for (const data of derivative) {
                ...
                if (data.length >= 2) {
                    ...
                    for (const childTag of childsTag) {
                        ...
                        if (!childNode) throw new Error(`Unknow derivative ${childTag}`);
            ...
            if (topNode.childs.length === 0) throw new Error(`Missing top nodes`);
            return topNode;
        }

        loadSkillTreeInfo(actorId: number, allNodes: Map<string, SkillTreeNode>) {
            ...
            for (const node of allNodes.values()) {
                if (!node.info()) throw new Error(`Node ${node.tag()} is missing node info`);
            }
        }
    }

    export class SkillTreeData {
        protected _actorSp: Map<number, SkillDataType>;
        protected _topNodes: Map<string, SkillTreeTopNode>;
        protected _allTypes: Map<number, SkillDataType[]>;
        ...
        setSp(actorId: number, sp: any) {
            this._actorSp.set(actorId, sp);
        }

        gainSp(actorId: any, sp: any) {
            const nowSp = this.sp(actorId);
            this.setSp(actorId, nowSp + sp);
        }

        topNode(type: SkillDataType): SkillTreeTopNode | undefined {
            return this._topNodes.get(type.skillTreeTag());
        }

        setTopNode(type: SkillDataType, topNode: SkillTreeTopNode) {
            this._topNodes.set(type.skillTreeTag(), topNode);
        }

        types(actorId: number): SkillDataType[] {
            return this._allTypes.get(actorId)!;
        }

        enableTypes(actorId: number) {
            if (SkillTreeManagementByClasses) {
                const actor = $gameActors.actor(actorId)!;
                const classId = actor.currentClass().id;
                return this.types(actorId).filter((type: { enabled: () => any; classId: () => number[]; }) => type.enabled() && type.classId().indexOf(classId) >= 0);
            } else {
                return this.types(actorId).filter((type: { enabled: () => any; }) => type.enabled());
            }
        }

        setTypes(actorId: number, types: SkillDataType[]) {
            this._allTypes.set(actorId, types);
        }

        totalSp(type: SkillDataType) {
            let sp = 0;
            for (const node of this.getAllNodesByType(type).values()) {
                if (node.isOpened()) sp += node.needSp();
            }
            return sp;
        }

        skillReset(type: SkillDataType) {
            for (const node of this.getAllNodesByType(type).values()) {
                if (node.isOpened()) node.close();
            }
        }

        totalSpAllTypes(actorId: number) {
            let sp = 0;
        ...
        skillResetAllTypes(actorId: number) {
            for (const type of this.enableTypes(actorId)) {
                this.skillReset(type);
            }
        }

        copyTree(dstType: SkillDataType, srcType: SkillDataType) {
        ...
        forceOpenNode(type: any, skillId: any) {
            for (const node of this.topNode(type)!.getAllChilds()) {
        ...
        makePoint(type: any, mode: string) {
        ...
        getAllNodesByType(type: SkillDataType): Map<string, SkillTreeNode> {
            const nodes = new Map();
            for (const node of this.topNode(type)!.getAllChilds()) {
    ...
    export class SkillTreeManager {
        protected _actorId!: number;
        protected _type!: SkillDataType | null;
        protected _selectNode!: SkillTreeNode | null;
        ...
        selectTopNode(topNode: { child: (arg0: number) => any; }) {
        ...
        setType(type: SkillDataType) {
            this._type = type;
        }

        setActorId(actorId: number) {
            this._actorId = actorId;
        }
        ...
        searchNode(xWay: number, yWay: number) {
            const nodes: SkillTreeNode[] = [...this.getAllNodes().values()];
            const selectNode: any = this._selectNode;
            if (xWay !== 0) {
    ...
    export class Scene_SkillTree extends Scene_MenuBase {
        protected _skillTreeManager!: SkillTreeManager;
        protected _interpreter!: Game_Interpreter;
        protected _windowTypeSelect!: Window_TypeSelect;
        protected _windowActorInfo!: Window_ActorInfo;
        protected _windowSkillTree!: Window_SkillTree;
        protected _windowSkillTreeNodeInfo!: Window_SkillTreeNodeInfo;
        protected _windowNodeOpen!: Window_NodeOpen;
        protected _backgroundSprite2!: Sprite;
        ...
        getBackgroundImage2(actorId: number) {
            const img2 = BackgroundImage.BackgroundImage2.find((img2: { ActorId: any; }) => img2.ActorId === actorId);
        ...
        applyMapDatas() {
            for (const skillTreeName of $skillTreeMapLoaders.keys()) {
                for (const actorId of $skillTreeData.actorIds()) {
                    ...
                    const type = $skillTreeData.types(actorId).find((t: { skillTreeName: () => string; }) => t.skillTreeName() === skillTreeName);
                    if (!type) continue;
        ...
        startCommonEvent(commonEventId: number) {
        ...
        getSkillTreeTypes(): SkillDataType[] {
    ...
    export class Window_TypeSelect extends Window_Command {
        protected _windowRect!: Rectangle;
        protected _types!: SkillDataType[];

        constructor(rect: Rectangle, types: SkillDataType[]);

        constructor(...args: [Rectangle]) {
            super(...args);
        }

        initialize(rect: Rectangle, types: SkillDataType[]) {
        ...
        drawItem(index: number) {
            ...
            if (iconIndex) {
                this.drawItemName(({ name: this.commandName(index), iconIndex: iconIndex } as any), rect.x, rect.y, this.width);
            } else {
        ...
        updateHelp() {
            ...
            if (type) description = type.helpMessage();
    ...
    export class Window_ActorInfo extends Window_Base {
        protected _actorId!: number;

        constructor(rect: Rectangle, actorId: number);

        constructor(...args: [Rectangle]) {
            super(...args);
        }

        initialize(...args: [Rectangle, number]): void {
            const [rect, actorId]: [Rectangle, number] = args;
            this._actorId = actorId;
            if (Utils.RPGMAKER_NAME === "MZ") {
                super.initialize(rect);
            } else {
    ...
    export class Window_SkillTreeNodeInfo extends Window_Base {
        protected _skillTreeManager!: SkillTreeManager;

        constructor(rect: Rectangle, skillTreeManager: SkillTreeManager);

        constructor(...args: [Rectangle]) {
            super(...args);
        }

        initialize(rect: Rectangle, skillTreeManager: SkillTreeManager) {
        ...
        drawCost(drawLine: number, textWidth: number, costTypes: string[]) {
        ...
        drawNeedSp(drawLine: number, textWidth: number) {
            const selectNode = this._skillTreeManager.selectNode();
        ...
        drawNeedGold(drawLine: number, textWidth: number) {
            const selectNode = this._skillTreeManager.selectNode();
        ...
        drawNeedVariables(drawLine: number, textWidth: number) {
            const selectNode = this._skillTreeManager.selectNode();
        ...
        drawNeedItems(drawLine: number, textWidth: number) {
            const selectNode = this._skillTreeManager.selectNode();
    ...
    export class Window_SkillTree extends Window_Selectable {
        protected _skillTreeManager!: SkillTreeManager;
        protected _windowTypeSelect!: Window_TypeSelect;
        protected _windowSkillTreeNodeInfo!: Window_SkillTreeNodeInfo;
        protected _viewSprite!: Sprite;
        protected _bitmapCache!: Bitmap | null;
        protected _skillTreeView!: SkillTreeView;
        protected _drawState!: string;
        protected _touchSelected!: boolean;
        protected _stayCount!: number;

        constructor(rect: Rectangle, skillTreeManager: SkillTreeManager, windowTypeSelect: Window_TypeSelect, windowSkillTreeNodeInfo: Window_SkillTreeNodeInfo);

        constructor(...args: [Rectangle]) {
            super(...args);
        }

        initialize(rect: Rectangle, skillTreeManager: SkillTreeManager, windowTypeSelect: Window_TypeSelect, windowSkillTreeNodeInfo: Window_SkillTreeNodeInfo) {
            this._skillTreeManager = skillTreeManager;
        ...
        checkHitNode(x: number, y: number): SkillTreeNode | null {
    ...
    export class Window_NodeOpen extends Window_Command {
        ...
        initialize(rect: Rectangle, skillTreeManager: SkillTreeManager): void;

        initialize(...args: [Rectangle, SkillTreeManager]) {
            const [rect, skillTreeManager]: [Rectangle, SkillTreeManager] = args;
            this._windowRect = rect;
            this._skillTreeManager = skillTreeManager;
            if (Utils.RPGMAKER_NAME === "MZ") {
                super.initialize(rect);
    ...
    export class SkillTreeView {
        ...
        constructor(skillTreeManager: any, windowWidth: any, windowHeight: any) {
            this._skillTreeManager = skillTreeManager;
            this._windowWidth = windowWidth;
            this._windowHeight = windowHeight;
        }

        static getPixelXY(point: Point) {
        ...
        nodeDiff(node1: SkillTreeNode, node2: SkillTreeNode) {
        ...
        drawLine(bitmap: Bitmap, x1: any, y1: any, x2: any, y2: any, color: any) {
        ...
        drawRect(bitmap: Bitmap, style: string, x: number, y: number, width: number, height: number, rectLineWidth: number) {
            const ctx = bitmap.context;
        ...
        drawIcon(dstBitmap: Bitmap, iconBitmap: Bitmap, x: number, y: number, opacity: number = 255) {
    ...
    Scene_Menu.prototype.createCommandWindow = function(this: Scene_Menu) {
    ...
    Scene_Menu.prototype.onPersonalOk = function(this: Scene_Menu) {
    ...
    Window_MenuStatus.prototype.processOk = function() {
        ...
        if (scene instanceof Scene_Menu && (scene as any)._commandWindow.currentSymbol() === "skillTree") {
    ...
    Game_Party.prototype.itemCount = function(this: Game_Party, itemId) {
        return this._items[itemId] ? this._items[itemId] : 0;
    };

    Game_Party.prototype.weaponCount = function(this: Game_Party, weaponId) {
        return this._weapons[weaponId] ? this._weapons[weaponId] : 0;
    };

    Game_Party.prototype.armorCount = function(this: Game_Party, armorId) {
    ...
    Game_Troop.prototype.spTotal = function() {
        return this.deadMembers().reduce((r, enemy) => {
            return r + (enemy as Game_Enemy).sp();
        }, 0);
    };
    ...
    BattleManager.makeRewards = function() {
        ...
        if (EnableGetSpWhenBattleEnd) (this as any)._rewards.sp = $gameTroop.spTotal();
    };

    const _BattleManager_gainRewards = BattleManager.gainRewards;
    BattleManager.gainRewards = function() {
        _BattleManager_gainRewards.call(this);
        if (EnableGetSpWhenBattleEnd) (this as any).gainSp();
    };

    (BattleManager as any).gainSp = function() {
    ...
    (BattleManager as any).displaySp = function() {
    ...
    Game_Actor.prototype.changeExp = function(this: Game_Actor, exp, show) {
    ...
    Game_Actor.prototype.levelUp = function(this: Game_Actor) {
    ...
    Game_Actor.prototype.displayLevelUp = function(this: Game_Actor, newSkills) {
