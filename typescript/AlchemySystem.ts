...
declare interface Window_MenuCommand {
    isEnabledAlchemy(): boolean;
}


const AlchemySystemPluginName = document.currentScript ? decodeURIComponent((document.currentScript as HTMLScriptElement).src.match(/^.*\/(.+)\.js$/)![1]) : "AlchemySystem";

namespace AlchemySystem {
    ...
    const EnabledMenuAlchemy: boolean = PP.EnabledMenuAlchemy;
    const EnabledAlchemySwitchId: number = PP.EnabledAlchemySwitchId;
    const EnabledCategoryWindow: boolean = PP.EnabledCategoryWindow;
    const EnabledGoldWindow: boolean = PP.EnabledGoldWindow;
    const DisplayItemCategory: boolean = PP.DisplayItemCategory == null ? true : PP.DisplayItemCategory;
    const DisplayWeaponCategory: boolean = PP.DisplayWeaponCategory == null ? true : PP.DisplayWeaponCategory;
    const DisplayArmorCategory: boolean = PP.DisplayArmorCategory == null ? true : PP.DisplayArmorCategory;
    const DisplayKeyItemCategory: boolean = PP.DisplayKeyItemCategory;
    const EnableIncludeEquipItem: boolean = PP.EnableIncludeEquipItem;

    const MaxNumMakeItem: number = PP.MaxNumMakeItem;
    const MaxMaterials: number = PP.MaxMaterials;

    const MakeItemSeFileName: string = PP.MakeItemSeFileName;
    const MakeItemSeVolume: number = PP.MakeItemSeVolume;
    const MakeItemSePitch: number = PP.MakeItemSePitch;
    const MakeItemSePan: number = PP.MakeItemSePan;

    const MenuAlchemyText: string = PP.MenuAlchemyText;
    const NeedMaterialText: string = PP.NeedMaterialText;
    const NeedPriceText: string = PP.NeedPriceText;
    const TargetItemText: string = PP.TargetItemText;
    const NoteParseErrorMessage: string = PP.NoteParseErrorMessage;
    ...
    let $recipes: AlchemyRecipe[] | null = null;

    export class PartyItemUtils {
        static partyItemCount(itemInfo: ItemInfo) {
            let count = this._partyItemCountWithoutEquips(itemInfo);
            if (EnableIncludeEquipItem) {
                if (itemInfo.type === "weapon") {
                    count += this._allPartyEquipWeapons().filter((item: { id: any; }) => item && item.id === itemInfo.id).length;
                } else if (itemInfo.type === "armor") {
                    count += this._allPartyEquipArmors().filter((item: { id: any; }) => item && item.id === itemInfo.id).length;
                }
            }
            return count;
        }

        static _partyItemCountWithoutEquips(itemInfo: ItemInfo) {
            return $gameParty.numItems(itemInfo.itemData());
        }

        static _allPartyEquipWeapons() {
            const equipSlotItems = $gameParty.members().flatMap((actor: { _equips: any; }) => actor._equips);
            const weaponItems = equipSlotItems.filter((item: { isWeapon: () => any; itemId: () => number; }) => item.isWeapon() && item.itemId() > 0);
            return weaponItems.map((item: { object: () => any; }) => item.object());
        }

        static _allPartyEquipArmors() {
            const equipSlotItems = $gameParty.members().flatMap((actor: { _equips: any; }) => actor._equips);
            const armorItems = equipSlotItems.filter((item: { isArmor: () => any; itemId: () => number; }) => item.isArmor() && item.itemId() > 0);
            return armorItems.map((item: { object: () => any; }) => item.object());
        }

        static gainPartyItem(itemInfo: ItemInfo, gainCount: number) {
        ...
        static _purgePartyEquipItem(itemInfo: { type: string; id: any; }, purgeCount: number) {
    ...
    class ItemInfo {
        ...
        static fromParams(params: { Type: string; ItemId: any; WeaponId: any; ArmorId: any; }) {
            let itemInfo;
            if (params.Type === "Item") {
    ...
    class Material {
        ...
        protected _count: any;

        constructor(itemInfo: ItemInfo, count: any) {
    ...
    class AlchemyRecipe {
        ...
        protected _targetItemInfo: ItemInfo;

        static fromRecipeData(recipeData: { material: any; target: any[]; price: any; }) {
            const materials: any = {};
        ...
        static fromRecipeDataV2(recipeDataV2: { Materials: any; TargetItemInfo: any; Price: any; }) {
            const materials: any = {};
        ...
        constructor(materials: {}, price: any, targetItemInfo: ItemInfo) {
        ...
        needItemCount(itemInfo: { tag: () => string | number; }) {
            return this._materials[itemInfo.tag()].count;
        }

        hasItemCount(itemInfo: ItemInfo) {
        ...
        makeItem(targetItemCount: number) {
            for (const tag in this._materials) {
    ...
    class Scene_Alchemy extends Scene_MenuBase {
        protected _windowSelectRecipes!: any;
        protected _windowRecipeDetail!: any;
        protected _categoryWindow!: any;
        protected _goldWindow!: Window_Gold_MZMV;
        protected _numberWindow!: Window_AlchemyNumber;
        protected _windowMakeItemYesOrNo!: any;
        ...
        parseRecipeData(item: { note: string; }) {
            const recipeDatas = [];
    ...
    class Window_SelectRecipes extends Window_Selectable {
        protected _recipes!: AlchemyRecipe[];
        ...
        recipesByCategory() {
            return $recipes!.filter((recipe) => {
                if (recipe.targetItemInfo().type === "item") {
                    const itemData = recipe.targetItemInfo().itemData() as any;
    ...
    class Window_RecipeDetail extends Window_Base {
        protected _numberWindow!: Window_AlchemyNumber | null;
        protected _recipe!: AlchemyRecipe;
        ...
        setNumberWindow(numberWindow: any) {
        ...
        totalPriceYOfs(lines: number) {
            return this.itemPadding() + lines * this.lineHeight() + this.lineHeight() * 2 + 20;
        }

        drawItemNumber(needItemCount: number, hasItemCount: any, x: number, y: number, width: number | undefined) {
