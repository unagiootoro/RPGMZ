...
(function (DotMoveSystem) {
    ...
    DotMoveSystem.PopupParameter = PopupParameter;
    class PluginParams {
        constructor() {
    ...
    DotMoveSystem.PluginParams = PluginParams;
    ...
    DotMoveSystem.DecideTriggerArea = DecideTriggerArea;
    ...
    class Game_Player_Mixin extends Game_Player {
        ...
        checkDecideTriggerEventInternal(x, y, d) {
            ...
            for (const result of this.mover().checkHitCharacters(x, y, d)) {
                if (!(result.targetObject instanceof DecideTriggerArea))
                    continue;
    ...
    class Game_Event_Mixin extends Game_Event {
        ...
        resetPopupSetting() {
            ...
            if (notePopupIcon)
                this._popupIcon = parseInt(notePopupIcon);
            const notePopupBalloon = this.getAnnotationValue("popupBalloon", 0);
            if (notePopupBalloon)
                this._decidableBalloonId = parseInt(notePopupBalloon);
            const notePopupImage = this.getAnnotationValue("popupImage", 0);
            if (notePopupImage)
                this._popupImageFileName = notePopupImage.replace(/^\s+/, "");
            const notePopupWindowText = this.getAnnotationValue("popupWindowText", 0);
            if (notePopupWindowText)
                this._popupWindowText = notePopupWindowText.replace(/^\s+/, "");
            const notePopupWindowTextFontSize = this.getAnnotationValue("popupWindowTextFontSize", 0);
            if (notePopupWindowTextFontSize)
                this._popupWindowTextFontSize = parseInt(notePopupWindowTextFontSize);
            const notePopupWindowTextFontFace = this.getAnnotationValue("popupWindowTextFontFace", 0);
            if (notePopupWindowTextFontFace)
                this._popupWindowTextFontFace = notePopupWindowTextFontFace.replace(/^\s+/, "");
    ...
    class Game_Map_Mixin extends Game_Map {
        allCharacters() {
            ...
            for (const event of this.events()) {
                characters.push(event.dotMoveTempData().decideTriggerArea);
            }
            return characters;
        }
    }
    ...
    DotMoveSystem.Sprite_Icon = Sprite_Icon;
    ...
    DotMoveSystem.Window_Popup = Window_Popup;
